#include "RentsServerHandler.h"

#include <logger/LoggerFactory.h>
#include <router/router.h>

RentsServerHandler::RentsServerHandler(const IResponseFactoryPtr &respFactory) : m_respFactory(respFactory)
{
}
...
void RentsServerHandler::HandleRequest(const std::shared_ptr<IRequest> &req)
{
    ...
    if (route)
    {
        ...
        {
            LoggerFactory::GetLogger()->LogError((std::string("[rents]: unhandled exception: ") + e.what()).c_str());
...
void RentsServerHandler::MakeResponse(const std::shared_ptr<IResponse> &resp)
{
    resp->copy(m_resp);
